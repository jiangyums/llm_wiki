 ...
 * http://localhost` header (NOT a webview-injected
 * `http://tauri.localhost`, and NOT the server's own URL — see
 * `localLlmOriginHeader` in llm-providers.ts for why the origin is
 * fixed to localhost).
 *
...
describe("streamChat against a fake Ollama server (real TCP)", () => {
  it(
    ...
    async () => {
      const server = await startFakeOllamaServer({
        ...
        selfOrigin: "", // set after we know the fixed origin below
      })
      try {
        // The client deliberately sends `Origin: http://localhost`
        // (see llm-providers.ts localLlmOriginHeader) regardless of
        // the actual server URL — it's always in Ollama's default
        // OLLAMA_ORIGINS allowlist.
  ...
  it(
    ...
    async () => {
      ...
      try {
        // The custom branch also goes through localLlmOriginHeader(),
        // so the fixed http://localhost origin is sent even though the
        // server URL differs from it.
        server.setRejectMode({ kind: "accept-same-origin", selfOrigin: "http://localhost" })
